...
export function getHueBleStatus(): HueBleConnectionStatus {
  const connected = Boolean(hueBleSession?.server.connected)
  return {
...
export async function connectHueBle(): Promise<HueBleConnectionStatus> {
  ...
  if (hueBleSession?.server.connected) {
  ...
  try {
    ...
    const service = await server.getPrimaryService(HUE_BLE_LIGHT_SERVICE)
    ...
    hueBleSession = null
    hueBleConnecting = false
    hueBleMessage = error instanceof Error
...
export async function applyHueBleMode(modeKey: HueMode) {
  ...
  if (!session?.server.connected) return { applied: false, reason: 'not_connected' as const }
