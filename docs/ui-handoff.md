...
| `ELEVENLABS_API_KEY` | 허브 음성 응답 | 음성 재생 없이 텍스트·기기 실행만 유지 |
| `ELEVENLABS_VOICE_ID` | 허브 목소리 지정 | ElevenLabs 음성 생성 제한 |
...
| 환경변수 | 기능 | 필수 여부 |
| --- | --- | --- |
| `NEXT_PUBLIC_DEMO_HUSBAND_ID` | 남편 데모 사용자 ID 호환 값 | 현재 최종 3화면 API는 직접 조회하지 않음 |
...
### 최종 시연에서 접근하는 Supabase 데이터

| 구분 | 테이블 또는 Storage |
| --- | --- |
| 공유 상태·허브 케어 | `mode_runs` |
| 실제 공기청정기 성공 로그 | `device_events` |
| AI 다이어리 | `diary_entries` |
| 초음파 성장 기록 | `ultrasound_records`, `ultrasound-images` |
| 다이어리·아침 안내 문맥 | `users`, `symptom_logs`, `moods` |
| 역할별 안내 저장 | `messages`, `daily_cards` |
| 허브 호환 조회·구독 | `alerts`, `hearts` |

자세한 API별 읽기·쓰기는 `docs/database-map.md`를 참고하세요.
