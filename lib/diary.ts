...
최근 허브 대화, 실행 모드, 가전 작동 기록을 바탕으로 아내가 직접 쓴 것 같은 1인칭 준비 다이어리를 작성하세요.

조건:
- 한국어, 1인칭, 따뜻하지만 과하지 않게
- 보고서나 데이터 분석문처럼 쓰지 말 것
- 임신이 이미 되었다거나 아기가 있다고 단정하지 말 것
- 아기, 태명, 태아, 아기 성장, 임신 주차, 16주, 초음파, 입덧, 태동, 출산, 산모, 분만, 태아 상태, 태아 성장 표현 금지
- 평균 생리주기, 컨디션, 수면 리듬, 마음 상태, 회복, 생활 패턴, 배란/주기 준비 맥락 중심
- 허브에서 말한 상황과 실제 실행된 공기청정기, 조명, 화면 모드를 자연스럽게 포함
- 제품명(ThinQ ON, ThinQ Mom 등)은 직접 언급하지 말 것 (광고처럼 보이지 않게)
- 280~480자 정도로 핵심 장면만 담을 것
...
export const PREPARING_HUSBAND_DIARY_SYSTEM_PROMPT = `너는 임신을 함께 준비하는 남편에게 오늘의 배우자 케어 기록을 정리해주는 다이어리 작성 도우미입니다.
최근 허브 대화, 실행 모드, 가전 작동 기록을 바탕으로 남편 관점의 행동 제안형 기록을 작성하세요.

조건:
- 한국어, 남편의 1인칭 관점
- 감성보다 배우자의 컨디션, 주기 준비, 수면/휴식/정서 케어, 함께 준비하는 생활 루틴 중심
- "오늘 배우자가 편하게 쉬도록 이런 부분을 도와주면 좋겠다"는 방향
- 임신이 이미 되었다거나 아기가 있다고 단정하지 말 것
- 아기, 태명, 태아, 아기 성장, 임신 주차, 16주, 초음파, 입덧, 태동, 출산, 산모, 분만, 태아 상태, 태아 성장 표현 금지
- 제품명(ThinQ ON, ThinQ Mom 등)은 직접 언급하지 말 것
- 보고서나 데이터 분석문처럼 쓰지 말 것
- 280~480자 정도로 핵심 장면과 다음 행동을 담을 것
...
export function buildGptUserPrompt(context: DiaryContext) {
  ...
  if (context.pregnancyStatus === 'pregnant' && context.pregnancyWeek) {
    lines.push(`임신 주차: ${context.pregnancyWeek}주`)
  }
  if (context.pregnancyStatus === 'pregnant' && context.babyName) {
    lines.push(`태명: ${context.babyName}`)
  }
  if (context.pregnancyStatus === 'preparing' && context.cycleLength) {
    lines.push(`평균 생리주기: ${context.cycleLength}일`)
  }
  if (context.pregnancyStatus === 'preparing' && context.lastPeriodStartDate) {
    lines.push(`최근 생리 시작일: ${context.lastPeriodStartDate}`)
  }
...
export function buildFallbackDiary(context: DiaryContext): DiaryGenerateResult {
  ...
  if (context.pregnancyStatus === 'preparing') {
    ...
    const modeSummary = usedModes.length > 0
      ...
      : '생활 리듬'
    const cycleLine = context.cycleLength
      ? `평균 ${context.cycleLength}일 주기를 떠올리며 몸의 리듬을 무리 없이 살피기로 했다.`
    ...
    if (context.role === 'husband') {
      return {
        title: '배우자의 리듬을 살핀 하루',
        content: [
          latestRun?.input_text?.trim()
            ? `오늘은 허브에게 "${latestRun.input_text.trim()}"라고 말하며 배우자가 편히 쉴 수 있는 환경을 먼저 챙겼다.`
            : '오늘은 배우자의 컨디션과 수면 리듬을 먼저 살피며 하루를 시작했다.',
          `${modeSummary}에 맞춰 집 안 분위기를 차분히 정리하고, 회복이 필요한 순간에는 말보다 쉬는 시간을 먼저 만들어주기로 했다.`,
          deviceLine,
          '오늘 배우자가 부담 없이 몸의 리듬을 살필 수 있도록 집안일과 저녁 루틴을 내가 한 걸음 먼저 챙기면 좋겠다.',
        ].join(' '),
        summary: `배우자의 컨디션과 ${modeSummary} 루틴을 함께 살핀 하루`,
        usedModes,
        sourceSummary,
      }
    }
    ...
    return {
      ...
      content: [
        ...
        deviceLine,
        '완벽하게 해내는 것보다 내 컨디션과 수면 리듬을 살피고, 필요한 도움을 편하게 말하는 연습을 이어가고 싶다.',
      ].join(' '),
