...
- ThinQ Mom 또는 ThinQ ON은 자연스럽게 1회 정도만 언급
- 400~700자 정도
...
export function buildFallbackDiary(context: DiaryContext): DiaryGenerateResult {
  ...
  if (context.pregnancyStatus === 'preparing') {
    ...
    const deviceLine = deviceActions.length > 0
      ? `ThinQ ON이 ${deviceActions.slice(0, 2).join(', ')} 설정을 이어주어 집 안 분위기도 한결 편안해졌다.`
      : 'ThinQ ON이 공기와 화면 분위기를 차분하게 맞춰주어 준비하는 마음도 조금 가벼워졌다.'
  ...
  if (context.role === 'husband') {
    ...
    return {
      ...
      content: [
        ...
        'ThinQ Mom이 오늘의 대화와 케어를 정리해주니 다음에는 무엇을 살펴야 할지도 조금 더 선명해졌다.',
        '내일도 같은 편에서 서두르지 않고 배우자의 속도에 맞춰 함께하고 싶다.',
      ].join(' '),
